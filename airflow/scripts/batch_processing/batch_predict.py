...
# Explicitly correct environment handling
RUNNING_LOCALLY = False  # Set explicitly False in Airflow, True locally
...
for table in input_tables:
    ...
    if df_raw.empty:
        print(f"⚠️ Warning: '{table}' explicitly has no data. Skipping prediction.")
        continue

    df_transformed = transformer.transform(df_raw)

    if df_transformed.empty:
        print(f"⚠️ Warning: '{table}' explicitly became empty after transformation. Skipping prediction.")
        continue
    ...
    print(f"✅ Predictions for '{table}' explicitly saved to '{output_table_name}' in PostgreSQL")
    print(f"✅ Predictions for '{table}' explicitly saved to '{output_csv_path}' as CSV\n")
